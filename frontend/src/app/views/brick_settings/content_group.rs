...
use super::ICON_ADD_BRICK;
#[cfg(target_arch = "wasm32")]
use crate::app::ICON_RESET;
#[cfg(target_arch = "wasm32")]
    ...
    html! {
        <EditorGroup
            ...
            header={html! {
                <>
                    <IconButton
                        icon={Html::from_html_unchecked(AttrValue::from(ICON_ADD_BRICK))}
                        title="Add to tutorial"
                        onclick={props.on_add_to_tutorial.clone()}
                    />
                    <IconButton
                        icon={Html::from_html_unchecked(AttrValue::from(ICON_RESET))}
        ...
        >
            <div class={style::CONTENT_GROUP_SHELL} onclick={close_context_menu_click}>
                ...
                if let Some((x, y)) = *context_menu {
                    <ContextMenu x={x} y={y} onmousedown={keep_focus_on_menu.clone()}>
                        <ContextMenuItem onmousedown={on_variable} disabled={!has_selection}>
                            {"Make variable"}
                        </ContextMenuItem>
                        <ContextMenuItem onmousedown={on_dropdown} disabled={!has_selection}>
                            {"Make dropdown"}
                        </ContextMenuItem>
                    </ContextMenu>
                }
