...
fn split_content_lines(
    ...
    brick: &BaseBrick,
    max_width: f32,
    dropdown_max_width: f32,
) -> Vec<ContentLine> {
    let mut lines = Vec::new();

    for raw_line in content.split('\n') {
        let line_width = if line_has_dropdown(raw_line) {
            dropdown_max_width
        } else {
            max_width
        };
        lines.extend(
            wrap_content_line(raw_line, brick, line_width)
                .into_iter()
...
pub trait Brick: Deref<Target = BaseBrick> + DerefMut + SVGRenderable + Pixmap {
    ...
    fn parse_content(&self) -> String {
        ...
        let triangle_x = dropdown_triangle_x(brick_width as f32, &scale);
        let dropdown_text_width = (triangle_x - offset_x).max(0.0);
        let lines = split_content_lines(content, self, available_width, dropdown_text_width);

        let svg_lines = lines.iter().enumerate().map(|(index, line)| {
            let ContentLine::Plain(line) = line;
            let line_content = parse_line(line, self);
            let line_triangle = if line_has_dropdown(line) {
                format!(
                    "<g transform=\"translate({} 0)\">{}</g>",
                    triangle_x,
                    render_line_dropdown_triangle(&self.color_scheme, &scale)
                )
            } else {
                String::new()
            };
...
mod tests {
    ...
    #[test]
    fn dropdown_triangle_does_not_follow_text_x_offset() {
        let mut brick = BrickH1Base::default();
        brick.content = "_choice_".to_string();
        let (brick_width, _) = brick.get_dimensions();
        let triangle_x = dropdown_triangle_x(brick_width as f32, &brick.scale);
        let triangle_group = format!("<g transform=\"translate({triangle_x} 0)\"><polygon");

        brick.offset.0 = 0.05;
        let first_text_x = brick.offset.0 * brick_width as f32;
        let first = brick.parse_content();

        brick.offset.0 = 0.2;
        let second_text_x = brick.offset.0 * brick_width as f32;
        let second = brick.parse_content();

        assert!(first.contains(&triangle_group));
        assert!(second.contains(&triangle_group));
        assert!(first.contains(&format!("<g transform=\"translate({first_text_x} 0)\">")));
        assert!(second.contains(&format!("<g transform=\"translate({second_text_x} 0)\">")));
    }
}
