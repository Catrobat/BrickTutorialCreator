...
#[derive(Clone, PartialEq)]
pub struct CustomColorEditState {
...
mod tests {
    ...
    fn custom(name: &str) -> ColorScheme {
        ColorScheme {
            name: name.to_string(),
            color: "#111111".to_string(),
            shade: "#222222".to_string(),
    ...
    #[test]
    fn custom_color_names_must_be_unique() {
        let mut model = BrickColorModel::new();

        assert!(model.add_custom_color(custom("Custom A")));
        assert!(!model.add_custom_color(custom("Custom A")));
    }

    #[test]
    fn custom_color_draft_trims_scheme_name() {
        let draft = CustomColorDraft {
            name: "  Custom A  ".to_string(),
            ..CustomColorDraft::default()
        };

        assert_eq!(draft.to_scheme().name, "Custom A");
    }
}
