...
pub fn dropdown_triangle_x(brick_width: f32, scale: &Scale) -> f32 {
    let fixed_right_inset = brick_width * DEFAULT_X_OFFSET;
    (brick_width - fixed_right_inset - dropdown_triangle_reserved_width(scale)).max(0.0)
}
...
pub fn render_line_dropdown_triangle(color_scheme: &ColorScheme, scale: &Scale) -> String {
    let font_size = font_size_from_scale(scale) * DROP_SCALE;
    let triangle_width = font_size * DROPDOWN_TRIANGLE_SCALE;
